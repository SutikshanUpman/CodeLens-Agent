...
> Paste a **GitHub profile** or a **repo/PR URL** → get an HR-lens review of your code, commit history, and project structure.

> 🚧 **Status: Active development** — architecture and core components are currently being built.

---

## Why

Students build projects for their resumes but rarely see what a recruiter actually notices: whether the commit history looks like real work or a one-shot upload, whether the code is actually clean, and whether the repo is organized like a real project. RecruiterEye automates that first pass, explains *why* something's flagged, and tells the student what to fix — a second pair of eyes before they apply.
...
4. **Authenticity** — takes the top 2–3 projects and checks their commit timing/diff sizes to flag unusual development patterns such as "written elsewhere and pasted in" vs. built over time.

A **chatbot** (Llama via Groq) sits on top so students can ask follow-up questions about their review.
...
* Chatbot is a separate graph/entry point, not part of this main graph — it reads `final_review` + RAG index as context.
...
| Payments            | Razorpay Payments API + Checkout (Test Mode)                                                                    |
...
RecruiterEye will use Razorpay to support an optional **Pro review tier** for deeper repository analysis.

* **Free review** — basic recruiter-lens analysis.
* **Pro review** — deeper analysis, including additional project insights and extended AI follow-up.
* **Razorpay Checkout** — handles the payment flow.
* **Razorpay Orders / Payment verification** — used to securely verify successful payments before unlocking Pro analysis.
* **Test Mode** — used during development and demonstration.

The core review remains accessible without payment; Razorpay is used to demonstrate a real AI-product monetization flow rather than being required for the basic analysis.
...
* [ ] RAGAS evaluation
* [ ] Chatbot for follow-up Q&A
* [ ] Web UI
* [ ] Razorpay integration — Pro tier, Checkout, Orders and payment verification
